...
import com.shasank.cms_form_extractor.dto.DiagnosisCode;
import com.shasank.cms_form_extractor.dto.ExtractionNote;
import com.shasank.cms_form_extractor.dto.FormExtractionResponse;
import com.shasank.cms_form_extractor.dto.ServiceLine;
...
public class FormExtractionService {
    ...
    public FormExtractionResponse extractFormData(MultipartFile file) throws IOException {
        ...
        FormExtractionResponse response = extractDataWithAgenticLoop(primaryImage);
     ...
     */
    private FormExtractionResponse extractDataWithAgenticLoop(BufferedImage image) throws IOException {
        String base64Image = fileProcessingUtil.imageToBase64(image);
        String prompt = promptBuilder.buildComprehensiveAnalysisPrompt();
        FormExtractionResponse response = null;

        int maxRetries = properties.getMaxRetries() != null ? properties.getMaxRetries() : 3;

        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            logger.info("Extraction attempt {} of {}", attempt, maxRetries);

            String ollamaResponse = ollamaClient.generateResponse(base64Image, prompt);
            response = parsingService.parseExtractionResponse(ollamaResponse);

            // Check if we should refine further
            if (response.getConfidenceScore() >= 0.85 || attempt == maxRetries) {
                logger.info("Extraction completed with confidence score: {}", response.getConfidenceScore());
                break;
            }

            // Refine for next attempt
            prompt = promptBuilder.buildRefinementPrompt(gson.toJson(response), attempt);
        }
